...
export function seedIfEmpty() {
  ...
  const userInsert = db.prepare(
    'INSERT INTO users (id, name, role, org_unit_id, created_at) VALUES (@id, @name, @role, @org_unit_id, @created_at)',
  )
  const users: { id: string; name: string; role: UserRole; org_unit_id: string }[] = [
    { id: 'u_admin_demo', name: '系统管理员（演示）', role: 'admin', org_unit_id: 'org_committee' },
    { id: 'u_secretary_demo', name: '支部书记（演示）', role: 'secretary', org_unit_id: 'org_branch_3' },
    { id: 'u_member_demo', name: '党员（演示）', role: 'member', org_unit_id: 'org_branch_3' },
    { id: 'u_member_2', name: '党员乙', role: 'member', org_unit_id: 'org_branch_3' },
    { id: 'u_member_3', name: '党员丙', role: 'member', org_unit_id: 'org_branch_1' },
  ]
  for (const u of users) userInsert.run({ ...u, created_at: ts })
